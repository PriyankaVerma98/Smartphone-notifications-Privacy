...
data <- read.csv("filtered_surveydata.csv") #df2 is a data frame
df2 <- data #copy into another dataframe
copy <- data
```
...
as.data.frame(((table(df))))  # the number of participants in each group
...
```{r}
#export the image
pdf(file="AppSensitivity2.pdf", width = 14, height = 9)
plot(image)
dev.off()
```
...
setnames(c, old = c("Q3.4_1","Q3.4_2", "Q3.4_3", "Q3.4_4", "Q3.4_5", "Q3.4_6", "Q3.4_7"), new = c('Spouse', 'Siblings', "Parents", "Children above 13", "Friends", "Colleagues", "Strangers"))
...
```{r}
#export data
pdf(file="device2_version2.pdf", width= 9)
plot(img)
dev.off()
```
...
df[] <- lapply(df, factor, levels = c("Never", "Rarely", "Sometimes", "Always", "N/A", "Not reported")) 
```
...
c$Scale <- factor(c$Scale, levels = c("Never", "Rarely", "Sometimes", "Always", "N/A", "Not reported"), ordered = TRUE)
class(c$Scale)

#remove row of "Not reported" and N/A
c <- c[-6, ]
c <- c[-5, ]
...
```{r}
#export image
pdf(file="shareBeh1_version4.pdf", width = 11)
plot(img)
dev.off()
```
...
print(tab_per[["0"]])
print(tab_per[18])
sprintf("atleast once %f percent N = %d", 100 - tab_per[["0"]]-tab_per[18], nrow(df)-tab_count[["0"]]-tab_count[18] )
```
...
print(tab_per[["0"]])
print(tab_per[24])
sprintf("atleast once %f percent N = %d", 100 - tab_per[["0"]]-tab_per[24], nrow(df)-tab_count[["0"]]-tab_count[24])
```
